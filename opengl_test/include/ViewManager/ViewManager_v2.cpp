...
void ViewManager::keyEvents(unsigned char key) {
	...
	case '-':
		...
		// Reset camera pose to default value
	case 'z':
	case 'Z':
		Reset();
		break;
        // Reset the default camera pose to hardcode default value and reset the camera pose to that default value.
    case 'x':
    case 'X':
        ResetDefaultValue();
        Reset();
...
void ViewManager::mouseMoveEvent(int x_cv_g, int y_cv_g)
{
	...
	if (lmbDown)
	{
		...
		*/
		...
		float factor = 0.002f;
		// vec3 _axis = (diff.x_cv_l * factor)*rotateYAxis + (diff.y * factor)*rotateXAxis;
        vec3 _axis = (diff.x * factor)*vec3(0,1,0) + (diff.y * factor)*vec3(1,0,0);
	...
	else if (midDown)
	{
        ...
        glm::vec3 delta_trans = (-diffUp + diffRight) * zoom * 3.0f * ( l2Norm(trans_world_at_cam + get_trans_view() ) + 0.2f)*0.3f;
...
void ViewManager::ResetDefaultValue(){
    ...
    glm::vec3 up(0, 1, 0);
    SetDefaultViewMatrix( lookAt(eyePosition, eyeLookPosition, up) );
    ...
    SetDefaultTansformMatrix( translationMatrix*rotationMatrix );

    //
    SetDefaultCameraModelInv( glm::mat4(1.0f) );
    // SetDefaultCameraModelInv( inverse(translate(glm::mat4(1.0), glm::vec3(69,-1396,6.67))) );
}
// The folowing functions can be used when camera mode changed
void ViewManager::SetDefaultCameraModelInv(glm::mat4 camera_model_inv_in){
    default_camera_model_inv = camera_model_inv_in;
}
void ViewManager::SetDefaultViewMatrix(glm::mat4 viewMatrix_in){
    default_viewMatrix = viewMatrix_in;
}
void ViewManager::SetDefaultTansformMatrix(glm::mat4 tansformMatrix_in){
    default_tansformMatrix = tansformMatrix_in;
}
