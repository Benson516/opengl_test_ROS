...
class SCENE_SW5 : public Scene
{
public:
	SCENE_SW5(std::string pkg_path_in);
...
SCENE_SW5::SCENE_SW5(std::string pkg_path_in)
{
	_camera_ptr.reset(new ViewManager());
    // _camera_ptr->assign_cal_viewport(&cal_viewport_w);
    // Layout
    //----------------------------------------//
    attach_cal_viewport_func_ptr(2, &cal_viewport_w2);
    switch_layout(0);
    //----------------------------------------//
    ...
    std::shared_ptr<rmImageBoard> _image_background_2_ptr(new rmImageBoard(_Assets_path, int(MSG_ID::camera_right_fore), false, true, true) );
