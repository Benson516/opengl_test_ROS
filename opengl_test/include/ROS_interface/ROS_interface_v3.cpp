...
void ROS_INTERFACE::_ROS_worker(){
    ...
    for (size_t _tid=0; _tid < _msg_type_2_topic_params[_msg_type].size(); ++_tid){
        ...
        // SPSC Buffer
        buffwr_list[_tmp_params.topic_id].reset(new buffwrString(_tmp_params.buffer_length) );
        // subs_id, pub_id
    ...
    for (size_t _tid=0; _tid < _msg_type_2_topic_params[_msg_type].size(); ++_tid){
        ...
        buffwr_list[_tmp_params.topic_id].reset(new buffwrtfGeoPoseStamped(_tmp_params.buffer_length) );
        // subs_id, pub_id
        if (_tmp_params.is_input){
            // Subscribe
            _pub_subs_id_list[_tmp_params.topic_id] = _subscriber_list.size();
    ...
    for (size_t _tid=0; _tid < _msg_type_2_topic_params[_msg_type].size(); ++_tid){
        MSG::T_PARAMS _tmp_params = _msg_type_2_topic_params[_msg_type][_tid];
        // SPSC Buffer
        buffwr_list[_tmp_params.topic_id].reset(new buffwrPointCloud2(_tmp_params.buffer_length) );
        // subs_id, pub_id
        if (_tmp_params.is_input){
            // Subscribe
            _pub_subs_id_list[_tmp_params.topic_id] = _subscriber_list.size();
    ...
    for (size_t _tid=0; _tid < _msg_type_2_topic_params[_msg_type].size(); ++_tid){
        MSG::T_PARAMS _tmp_params = _msg_type_2_topic_params[_msg_type][_tid];
        // SPSC Buffer
        buffwr_list[_tmp_params.topic_id].reset(new buffwrITRI3DBoundingBox(_tmp_params.buffer_length) );
        // subs_id, pub_id
        if (_tmp_params.is_input){
            // Subscribe
            _pub_subs_id_list[_tmp_params.topic_id] = _subscriber_list.size();
...
void ROS_INTERFACE::_String_CB(const std_msgs::String::ConstPtr& msg, const MSG::T_PARAMS & params){
    ...
    _tmp_s = msg->data;
    // put
    bool result = buffwr_list[params.topic_id]->put_void( &(_tmp_s), true, _time_in, false);
    // bool result = buffwr_list[params.topic_id]->put_void( &(msg->data), true, _time_in, false);
