...
void rmlv2PathPlan_navPath::update_GL_data( ROS_API &ros_api ){
    ...
    //
    if (!tf_successed){
        std::cout << "[rmlv2PathPlan_navPath] No tf got, cannot transform the path.\n";
        return;
    }
    //----------------------------------//
    // end Get tf
...
//
void rmlv2PathPlan_navPath::get_point3D_poly(const std::vector<glm::vec2> &param, double dT, glm::vec3 &point3D_out){
